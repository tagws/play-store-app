...
function setup() {
  ...
  monkey = createSprite(invisibleGround.x,invisibleGround.y);
  ...
  start1 = createSprite(width/2,height/2,0,0);
    start1.addImage(start);
  start1.visible = false;
  start1.scale = 0.040;
  
  
   reset1 = createSprite(width/2,height/2);
    reset1.addImage(reset);
    reset1.scale = 0.3;
  
  
  
  play = createSprite(width/2,height/2,0,0);
  play.addImage(play1);
  play.scale = 0.070
  
  
  
  mute = createSprite(width-50,50,0,0);
  mute.addImage(mute1);
  mute.scale = 0.070;
...
function draw() {
 ...
 reset1.x = 1000;
  ...
  if(gameState === "Start"){
    ...
    if(touches.lenght>0||mousePressedOver(start1)){
 ...
 if (back2.x < 0){
  ...
  if(touches.length > 0 || keyWentDown("space")&& monkey.y >=190){
